...
const shortTitle = movies.filter((film) => {
  return film.title.length < 5;
});
// console.log(shortTitle); question no 1

const longTitle = movies.filter((film) => {
  return film.title.length > 5;
});
// console.log(longTitle); question no 2
...
//console.log(count); question no 3

const extraTag = movies.forEach((film) => {
  if (film.rating >= 7) {
    film.tag = "Good";
...
//console.log(movies[0]);// displays the first item in the array to see if the tag is available. question no 4

const filterArray = movies.filter((film) => {
  return film.rating > 6;
});
const mapArray = filterArray.map((film) => {
  return film.rating;
});
//console.log(mapArray) question no 5

let countMovies = 0;
keywords = ["Surfer", "Alien", "Benjamin"];
movies.forEach((film) => {
  for (let i = 0; i < keywords.length; i++) {
    if (film.title.toLowerCase().includes(keywords[i].toLowerCase())) {
      return (countMovies += 1);
    }
  }
});
//console.log(countMovies); question no6

let titleArray = [];
let titleFilter = movies.forEach((film) => {
  if (film.title.length > 0) {
    titleArray.push(film.title);
  }
});
let duplicatedWords = [];
const words = titleArray.forEach((item) => {
  duplicatedWords[item] = titleArray.filter((duplicate) => {
    return duplicate === item;
  }).length;
});
//console.log(duplicatedWords);
let num = movies.rating;
const reduceArray = movies.reduce((accumulator, num) => {
  return accumulator + num.rating;
}, 0);
